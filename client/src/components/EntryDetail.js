...
const EntryDetail = ({ token }) => {
  ...
  const stripContentEditable = (htmlContent) => {
    ...
    // Remove contenteditable attributes and resize handles
      ...
      <main className="main-content">
        <div className="entry-detail-container">
          ...
          <div className="entry-detail-content">
            ...
            <div className="description" dangerouslySetInnerHTML={{ __html: stripContentEditable(entry.content) }} />
            <div className="entry-meta">
                ...
                <div className="weather-details">
                  <h3>
                    ...
                    <span className="cloud-cover-title">
                      <i className="fas fa-cloud weather-icon"></i> {entry.weather.clouds?.all || "N/A"}%
                    </span>
                  </h3>
                  <div className="weather-columns">
                    <div className="weather-column">
                      <div className="weather-card">
                        <p><i className="fas fa-cloud-sun weather-icon"></i><strong>Main:</strong> <span>{entry.weather.weather[0]?.main || "N/A"}</span></p>
                        <p><i className="fas fa-info-circle weather-icon"></i><strong>Description:</strong> <span>{entry.weather.weather[0]?.description || "N/A"}</span></p>
                        <p><i className="fas fa-thermometer-half weather-icon"></i><strong>Temperature:</strong> <span>{((entry.weather.main?.temp - 273.15) * 9/5 + 32).toFixed(1)}°F</span></p>
                        <p><i className="fas fa-temperature-low weather-icon"></i><strong>Feels Like:</strong> <span>{((entry.weather.main?.feels_like - 273.15) * 9/5 + 32).toFixed(1)}°F</span></p>
                        <p><i className="fas fa-temperature-down weather-icon"></i><strong>Min Temp:</strong> <span>{((entry.weather.main?.temp_min - 273.15) * 9/5 + 32).toFixed(1)}°F</span></p>
                        <p><i className="fas fa-temperature-up weather-icon"></i><strong>Max Temp:</strong> <span>{((entry.weather.main?.temp_max - 273.15) * 9/5 + 32).toFixed(1)}°F</span></p>
                      </div>
                    </div>
                    <div className="weather-column">
                      <div className="weather-card">
                        <p><i className="fas fa-tachometer-alt weather-icon"></i><strong>Pressure:</strong> <span>{(entry.weather.main?.pressure * 0.02953).toFixed(2) || "N/A"} inHg</span></p>
                        <p><i className="fas fa-tint weather-icon"></i><strong>Humidity:</strong> <span>{entry.weather.main?.humidity || "N/A"}%</span></p>
                        <p><i className="fas fa-eye weather-icon"></i><strong>Visibility:</strong> <span>{entry.weather.visibility ? (entry.weather.visibility / 1609.34).toFixed(1) + " mi" : "N/A"}</span></p>
                        <p><i className="fas fa-wind weather-icon"></i><strong>Wind Speed:</strong> <span>{(entry.weather.wind?.speed * 2.23694).toFixed(1) || "N/A"} mph</span></p>
                        <p><i className="fas fa-compass weather-icon" style={{ transform: `rotate(${entry.weather.wind?.deg || 0}deg)` }}></i><strong>Wind Direction:</strong> <span>{entry.weather.wind?.deg || "N/A"}°</span></p>
                        <p><i className="fas fa-wind weather-icon wind-gust"></i><strong>Wind Gust:</strong> <span>{entry.weather.wind?.gust ? (entry.weather.wind.gust * 2.23694).toFixed(1) + " mph" : "N/A"}</span></p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <p><strong>Coordinates:</strong> {entry.geolocation?.lat && entry.geolocation?.lng ? `Lat: ${parseFloat(entry.geolocation.lat).toFixed(4)}°, Lng: ${parseFloat(entry.geolocation.lng).toFixed(4)}°` : "Not available"}</p>
